...
[root@centos7-vm mai]\# fuser -v /mnt/mai
...
```bash
[root@centos7-vm mai]\# lsof -p 28629
...
```bash
[root@centos7-vm ~]\# kill -9 29372
[root@centos7-vm ~]\# umount /mnt/mai
```

## Решаем загадку исчезновения места на диске

```bash
[root@centos7-vm myfiles]\# mkdir myfiles
[root@centos7-vm myfiles]\# ls -l
total 1048516
...
[root@centos7-vm myfiles]\# test myfile
[root@centos7-vm myfiles]\# jobs -l
[root@centos7-vm myfiles]\# fg
bash: fg: current: no such job
^Z
[4]+  Stopped                 python test_write.py
[root@centos7-vm myfiles]\# bg
bash: bg: current: no such job
[root@centos7-vm myfiles]\# df
...
[root@centos7-vm myfiles]\# du ~/myfiles
524228  /root/myfiles
[root@centos7-vm myfiles]\# rm myfile
[root@centos7-vm myfiles]\# df
Filesystem     1K-blocks    Used Available Use% Mounted on
...
tmpfs             507484       0    507484   0% /sys/fs/cgroup
tmpfs             101500       0    101500   0% /run/user/1000
[root@centos7-vm myfiles]\# du sh ~/myfiles
du: cannot access ‘sh’: No such file or directory
524228  /root/myfiles
[root@centos7-vm myfiles]\#
```
...
```bash
[root@centos7-vm myfiles]\# strace -p 30193
strace: Process 30193 attached
--- stopped by SIGTSTP ---
[root@centos7-vm myfiles]\# lsof -p 30193
COMMAND   PID USER   FD   TYPE DEVICE   SIZE/OFF      NODE NAME
...
[root@centos7-vm myfiles]\# kill -9 30193
[root@centos7-vm myfiles]\# df
Filesystem     1K-blocks    Used Available Use% Mounted on
